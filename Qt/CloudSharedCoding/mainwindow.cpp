...
int MainWindow::local_project_id = -1; // ¨¦?????a????????¨¦????????????¡ã????
int MainWindow::local_file_id = -1;    // ¨¦?????a????????¨¦????????????¡ã????
...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent), ui(new Ui::MainWindow)
{
    ...
    // ¨¦????¡ì?¡ì?¨¦????????¨¦????¡ì????????????¨¦????¡ã?¡ä?
    EncodingTypeLabel->setText("        ?¡è¡ã??????¨¦?¡§??-?¡ä?¨¦???????????????¡ä?¨¦???-?TF-8 (????¡§??????????TF-8?????????¨¦???????¡ì¨¦??a??¨¦???-????¨¦????????¨¦??????¡ä?¨¦????????????-?????????¨¦????¡ã?¡ä?¨¦?????????????????¨¦???¡è????");
    ...
    //*************************************************************************************************
    connect(ui->actionNew_local_project, SIGNAL(triggered()), this, SLOT(newLocalProj()));   // ¨¦????????¨¦???????¡ä?¡è¡è??-?¡ä¡ã
    connect(ui->actionAdd_Local_File, &QAction::triggered, this, &MainWindow::addLocalFile); // ?¡§?????¡ì?¨¦???????¡ä¨¦????????
    connect(ui->actionOpenLocal_project, SIGNAL(triggered()), this, SLOT(openLocalProj()));  // ¨¦??????¡ä?¨¦???????¡ä?¡è¡è??-?¡ä¡ã
    connect(ui->actionSave, &QAction::triggered, this, &MainWindow::saveLocalProj);          // ?¡¦??????¡§¨¦???????¡ä?¡è¡è??-?¡ä¡ã
    //*************************************************************************************************
...
void MainWindow::deleteProFile()
{
    ...
    if (item->getType() == MyTreeItem::FILE)
    {
        ...
        QMessageBox::StandardButton result = QMessageBox::warning(this, "??-??????¨¦?????????", "¨¦????¡ìa??????¡ã????¨¦????????¨¦?????????" + file->file_name + "?");
    ...
    else
    {
        ...
        //        QMessageBox::StandardButton result = QMessageBox::warning(this,"??-??????¨¦?????????","¨¦????¡ìa??????¡ã????¨¦????????¨¦?????????"+dir->dir_name+"?");
        //        if(result!=QMessageBox::StandardButton::Ok)
        //            return;
...
void MainWindow::newLocalProj()
{
    ...
    connect(dialog->get_pushButton_new(), &QPushButton::clicked, this, [=]()
        ...
        else
        {
            ...
            {
                QMessageBox::critical(this,"¨¦????????","¨¦?????????¡è¡è??-?¡ä¡ã???????¡ì?");
            }
            dialog->close();
        } });
}
...
void MainWindow::openLocalProj()
{
    ...
    {
        QMessageBox::critical(this, tr("¨¦?????"), "??????¨¦??????¡è??¡ä?");
        return;
    }
    ...
    connect(dialog->get_pushButton_add(), &QPushButton::clicked, this, [=]()
        ...
        else
        {
            if(dialog->get_comboBox_current_index()==0)//.cpp
            {
               ...
               if(res==true)
                   QMessageBox::information(this,"¨¦?¡ä????¡ì?","?¡§?????¡ì?¨¦????????¨¦?¡ä????¡ì?");
               else
               {
                   QMessageBox::information(this,"???????¡ì?","?¡§?????¡ì?¨¦???????????????¡ì?");
                   return;
               }
...
void MainWindow::saveLocalProj()
{
    ...
    QMessageBox::information(this, "¨¦?¡ä????¡ì?", "?¡è¡è??-?¡ä¡ã?¡¦??????¡§¨¦?¡ä????¡ì?");
}
...
// ????????¡ã????????¡§?????¡§????????????????¡è??????¡¦????????-??????????????????????¡¦??????¡ã?????¡ãQStringList???????(?????¡ãtag1==0???????????¡¦??????"*.h"?????????tag==1???????????¡¦??????"*.cpp"??????)
